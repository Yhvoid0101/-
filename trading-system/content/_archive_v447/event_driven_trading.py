...
class EventDrivenTrading:
    ...
    def analyze(self) -> EventSignal:
        ...
        P0-4b 修复: 数据有效性检查, 防止基于空数据返回错误信号
        根因: 当 feeder (update_fear_greed/update_economic_data) 未被调用时,
              fear_greed_index=0, fed_funds_rate=0, cpi_yoy=0 等都是默认值
              analyze_fear_greed(0) 会触发 EXTREME_FEAR_BUY (极度恐惧买入)
              analyze_real_rate(0) 会触发 NEGATIVE_REAL_RATE_BUY (负实际利率买入)
              导致 40% event_driven Agent 基于空数据发出错误做多信号
        修复: 数据未更新时直接返回 HOLD 信号, 不进入后续分析
        """
        signal = EventSignal()
        signal.fear_greed_index = self.fear_greed_index
        signal.real_rate = self.calculate_real_rate()

        # P0-4b: 数据有效性检查 - fear_greed_index=0 表示 feeder 从未调用
        # event_calendar 为空 表示无宏观事件数据
        if self.fear_greed_index == 0.0 and not self.event_calendar:
            signal.signal_type = EventSignalType.HOLD
            signal.description = "数据未更新 (feeder 未调用), 返回 HOLD 避免错误信号"
            return signal

        # P0-4c 升级修复: event_calendar 为空时不产生信号
        # 根因: INTRADAY-GUARD 的 update_fear_greed feeder 会从价格历史推断 FGI (非0值),
        #       update_economic_data feeder 用默认值 (fed_funds=5.25, dxy=104.0 等),
        #       这绕过了 P0-4b 的 fear_greed_index==0 检查,
        #       导致 analyze_fear_greed(98.4)=EXTREME_GREED_SELL + analyze_dxy(104)=DXY_STRONG_SELL
        #       全部指向做空, 产出错误 SHORT conf=0.600 信号
        # 修复: event_driven_trading 是"宏观事件驱动"策略, 无宏观事件时应返回 HOLD
        #       不基于推断的 FGI 和默认经济数据产生信号 (假信号比无信号更危险)
        # 协调: 保留 INTRADAY-GUARD 的 FGI 推断逻辑 (让 sandbox 能喂数据),
        #       但在 analyze() 层面拦截无事件情况
        if not self.event_calendar:
            signal.signal_type = EventSignalType.HOLD
            signal.description = (
                "无宏观事件日历 (event_calendar 为空), 不产生事件驱动信号. "
                "FGI={:.1f} (推断值), real_rate={:.3f}".format(
                    self.fear_greed_index, self.calculate_real_rate()
                )
            )
            return signal
